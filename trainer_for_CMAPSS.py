...
from datasets.CMAPSS import prepare_tadj
...
class train_utils(object):
    ...
    def setup(self):
        ...
        self.dataloaders = {x: torch.utils.data.DataLoader(self.datasets[x], batch_size=args.batch_size,
                            ...
                            for x in ['train', 'val']}
        # Define the model
        self.tam = torch.from_numpy(self.tam).float().to(self.device)
        self.am = torch.from_numpy(self.am).float().to(self.device)
        self.model = getattr(models, args.model_name)(tadj=self.tam, sadj=self.am, device=self.device)
            ...
            for phase in ['train', 'val']:
                ...
                for batch_idx, (inputs, labels, work_condition, cycles) in enumerate(self.dataloaders[phase]):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        mse, phm_score, mra, mape = self.cal_acc(labels, logits)
                        ...
                        epoch_mra += mra * inputs.size(0)
                        epoch_mape += mape * inputs.size(0)
                        ...
                        if phase == 'train':
                            ...
                            batch_mra += mra * inputs.size(0)
                            batch_mape += mape * inputs.size(0)
                            ...
                            if step % args.print_step == 0:
                                ...
                                batch_mra = batch_mra / batch_count
                                batch_mape = batch_mape / batch_count
                                temp_time = time.time()
                                train_time = temp_time - step_start
                                step_start = temp_time
                                batch_time = train_time / args.print_step if step != 0 else train_time
                                sample_per_sec = 1.0 * batch_count / train_time
                                logging.info('Epoch: {} [{}/{}], {} Loss: {:.4f} MSE: {:.4f} RMSE: {:.4f} PHM: {:.4f} MRA: {:.4f} MAPE: {:.4f} {:.1f} examples/sec {:.2f} sec/batch'.format(
                                    epoch, batch_idx * len(inputs), len(self.dataloaders[phase].dataset), phase,
                                    batch_loss, batch_mse, math.sqrt(batch_mse), batch_phm_score, batch_mra, batch_mape,
                ...
                epoch_mra = epoch_mra / len(self.dataloaders[phase].dataset)
                epoch_mape = epoch_mape / len(self.dataloaders[phase].dataset)

                logging.info('Epoch: {} {}-Loss: {:.4f} {}-MSE: {:.4f}-RMSE: {:.4f}-PHM: {:.4f}-MRA: {:.4f}-MAPE: {:.4f}-Cost {:.1f} sec'.format(
                    epoch, phase, epoch_loss, phase, epoch_mse, math.sqrt(epoch_mse), epoch_phm_score, epoch_mra, epoch_mape,
                ...
                if phase == 'val':
                    ...
                    # epoch_rmse_score = math.sqrt(epoch_mse)
