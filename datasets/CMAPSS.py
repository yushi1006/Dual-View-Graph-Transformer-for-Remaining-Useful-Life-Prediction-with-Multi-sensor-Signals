...
def normalization_train_data(df, cols, type):
    for j in range(len(cols)):
        if type == '-1-1':
            min_value = df[cols[j]].min()
            max_value = df[cols[j]].max()
            df[cols[j]] = 2*(df[cols[j]] - min_value) / (max_value - min_value)-1
        elif type == 'z-score':
            mean_value = df[cols[j]].mean()
            std_value = df[cols[j]].std()
            df[cols[j]] = (df[cols[j]] - mean_value) / std_value
        else:
...
def normalization_test_data(df_train, df_test, cols, type):
    for j in range(len(cols)):
        if type == '-1-1':
            min_value = df_train[cols[j]].min()
            max_value = df_train[cols[j]].max()
            df_train[cols[j]] = 2*(df_train[cols[j]] - min_value) / (max_value - min_value)-1
            df_test[cols[j]] = 2*(df_test[cols[j]] - min_value) / (max_value - min_value)-1
        elif type == 'z-score':
            mean_value = df_train[cols[j]].mean()
            std_value = df_train[cols[j]].std()
            df_train[cols[j]] = (df_train[cols[j]] - mean_value) / std_value
            df_test[cols[j]] = (df_test[cols[j]] - mean_value) / std_value
        else:
            raise NameError('This normalization is not included!')
    return df_test
...
def local_load_data(data_path):
    raw_data = pd.read_csv(data_path, sep=r'\s+', header=None)
...
if __name__ == "__main__":
    train_data_path = 'D:/dataset/CMAPSSData/train_FD001.txt'
    test_data_path = 'D:/dataset/CMAPSSData/test_FD001.txt'
    test_RUL_path = 'D:/dataset/CMAPSSData/RUL_FD001.txt'
    adj_path = 'D:/dataset/CMAPSSData/adj_001_0_14.txt'
    data_path = [train_data_path, test_data_path, test_RUL_path, adj_path]
    am = prepare_adj(data_path)
    tam = prepare_tadj('D:/dataset/CMAPSSData/tadj.txt')
    datasets = {}
    ...
    datasets['test'] = CMAPSS(data_path=data_path, normalization_type='z-score',
                              win_len=30,  diff_order=0, train_mode='test')
